...
/**
 * GIF validation utilities for mock E2E tests (Selenium version)
 ...
 */
export async function urlToBuffer(driver: WebDriver, url: string): Promise<Buffer> {
  if (url.startsWith('data:image/gif;base64,')) {
    ...
    // Blob URL - need to fetch via driver context
    const base64 = await driver.executeScript<string>(`
      return (async function(blobUrl) {
...
export async function validateGifFromUrl(
  ...
  const metadata = await extractGifMetadata(driver, gifUrl);
...
export async function validateTextOverlay(
  ...
  const screenshot = await driver.findElement({ css: gifSelector }).takeScreenshot();
